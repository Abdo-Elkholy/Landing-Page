...
for (let i = 0; i < sectionsList.length; i++) {
  let li =
    document.createElement(
      "li"
    ); /*for every item in sectionsList the loop create new li and a element*/
  let link = document.createElement("a");
  link.href = `#section${
    i + 1
  }`; /* every section get his href by looping on every item in sectionsList array, then add 1 for every index in list because index in list start from 0*/
...
for (let x = 0; x < linksList.length; x++) {
  linksList[x].onclick = function (a) {
    ...
    document.querySelector(this.getAttribute("href")).scrollIntoView({
      behavior: "smooth",
      block: "end",
      inline: "nearest" /*behavior property to get smooth scrolling*/,
    });
  };
}
...
window.addEventListener("scroll", (_) => {
  /*use scroll event and execute the function, then use forEach to loop
...
window.addEventListener(`scroll`, (_) => {
  if (scrollY >= 500) {
    btn.style.display =
      "block"; /* the button appears when the above condition is true then the header and button disappear after the event occurs(the click) */
